...
#def filter_database_fullText(user_keywords, cond, cate, weeks):
    ...
    # Get the latest timestamp using raw SQL
    with connection.cursor() as cursor:
        cursor.execute(f"SELECT MAX(timestamp) FROM {NewsData._meta.db_table}")
        latest_timestamp_str = cursor.fetchone()[0]
        
        
    latest_timestamp = datetime.strptime(latest_timestamp_str, '%Y-%m-%d').timestamp()
    # Calculate start timestamp
    start_timestamp = latest_timestamp - timedelta(weeks=weeks)
    
    # Format timestamps for SQL query (if necessary)
    start_timestamp_str = start_timestamp.strftime('%Y-%m-%d')
    latest_timestamp_str = latest_timestamp.strftime('%Y-%m-%d')
    
    # Build the base SQL query
    table_name = NewsData._meta.db_table
    params = [start_timestamp_str, latest_timestamp_str]
    ...
    if id_list:
        queryset = NewsData.objects.filter(id__in=id_list)
    else:
...
def get_title_link_topk(queryset, k=25):
    ...
    news_items = queryset.values('category', 'title', 'link', 'photo')[:k]
...
def get_keyword_time_based_freq(queryset):
    # Extract all timestamps from the queryset
    timestamps = list(queryset.values_list('timestamp', flat=True))
    
    # Create DataFrame from the timestamps
    query_freq = pd.DataFrame({'timestamp_index': pd.to_datetime(timestamps), 'freq': [1 for _ in range(len(timestamps))]})
    
    # Group by timestamp and count occurrences
    data = query_freq.groupby(pd.Grouper(key='timestamp_index', freq='D')).sum()
...
Memory Efficiency: You're not loading all timestamps into memory
Less Code: No need for the intermediate pandas DataFrame conversion
Additional Notes:
TruncDate truncates a timestamptime field to its timestamp component, allowing you to group by day
...
def get_keyword_time_based_freq_with_orm(queryset):
    ...
    # Use timestamp string extraction (works on SQLite)
    # This uses Cast to convert timestamp field to string in YYYY-MM-DD format
    timestamp_counts = (
        queryset
        .annotate(day=Cast('timestamp', DateField()))  # Extract just the timestamp part
        .values('day')
    ...
    for item in timestamp_counts:
        ...
        if isinstance(item['day'], str):
            timestamp_str = item['day']
        else:
            timestamp_str = item['day'].strftime('%Y-%m-%d') # Convert to string 
...
def get_keyword_time_based_freq_raw_sql(queryset):
    ...
    table_name = NewsData._meta.db_table
    ids = tuple(queryset.values_list('id', flat=True))  # Convert to tuple
...
def get_daily_basis_sentiment_count(df_query, sentiment_type='pos', freq_type='D'):
    ...
    freq_df = pd.DataFrame({'timestamp_index': pd.to_datetime(df_query['timestamp']),
                             'frequency': [lambda_function(senti) for senti in df_query.sentiment]})
    # Group rows by the timestamp to the daily number of articles. 加總合併同一天新聞，篇數就被計算好了
    freq_df_group = freq_df.groupby(pd.Grouper(key='timestamp_index',freq=freq_type)).sum()
    
    # 'timestamp_index'為index(索引)，將其變成欄位名稱，inplace=True表示原始檔案會被異動
    freq_df_group.reset_index(inplace=True)
    
    # x,y，用於畫趨勢線圖
    xy_line_data = [{'x':timestamp.strftime('%Y-%m-%d'),'y':freq} for timestamp, freq in zip(freq_df_group.timestamp_index,freq_df_group.frequency)]
    return xy_line_data
