...
def get_userkey_data(request):
    ...
    try:
        response_from_sentiment = api_get_sentiment(request)
        ...
        print(f"Error calling api_get_sentiment: {e}")
        return{'error': 'Failed to get sentiment data.'}


    # (2)從內部取得聲量分布資料 get frequency data from internal module
    try:
        response_from_userkeyword = api_get_user_keyword(request)
        ...
        print(f"Error calling api_get_user_keyword: {e}")
